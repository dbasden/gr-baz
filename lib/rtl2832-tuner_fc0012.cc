/*
 * fc0012 tuner support for gr-baz
 *
 * Based on tuner_fc0012.c found as part of the (seemingly GPLed)
 * rtl2832u Linux DVB driver.
 *
 * Rewritten and ported by David Basden <davidb-sdr@rcpt.to>
 */
...
int FC0012_Write(baz_rtl_source_c* pTuner, uint8_t RegAddr, uint8_t Byte)
{
	...
	if (pTuner->rtl_i2c_write(FC0012_I2C_ADDR, data, 2) < 0)
		return FC0012_ERROR;
...
int FC0012_Read(baz_rtl_source_c* pTuner, uint8_t RegAddr, uint8_t *pByte)
{
	...
	if (pTuner->rtl_i2c_write(FC0012_I2C_ADDR, &data, 1) < 0)
		return FC0012_ERROR;
	if (pTuner->rtl_i2c_read(FC0012_I2C_ADDR, &data, 1) < 0)
		return FC0012_ERROR;
...
int FC0012_SetFrequency(baz_rtl_source_c* pTuner, unsigned long Frequency, unsigned short Bandwidth)
{
	...
	DEBUGF("FC0012_SetFrequency SUCCESS"); FC0012_Dump_Registers(pTuner);
	return FC0012_OK;
}
	...
	uint8_t msb, uint8_t lsb, uint8_t *pReadingValue)
{
	...
	for(i = lsb; i < (msb + 1); i++)
		mask |= 0x1 << i;
	if(FC0012_Read(pTuner, regAddr, &readingByte) != FC0012_OK)
		return FC0012_ERROR;

	*pReadingValue = (readingByte & mask) >> lsb;
	...
	uint8_t msb, uint8_t lsb, const uint8_t value)
{
	...
	writingByte |= (value << lsb) & mask;

	if(FC0012_Write(pTuner, regAddr, writingByte) != FC0012_OK)
		return FC0012_ERROR;
